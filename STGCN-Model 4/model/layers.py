...
class STConvBlock(nn.Module):
    ...
    def __init__(self, Kt, Ks, n_vertex, last_block_channel, channels, act_func, graph_conv_type, gso, bias, droprate):
        ...
        self.graph_conv1 = GraphConvLayer(graph_conv_type, channels[0], channels[1], Ks, gso, bias)
        self.tmp_conv2 = TemporalConvLayer(Kt, channels[1], channels[2], n_vertex, act_func)
        self.graph_conv2 = GraphConvLayer(graph_conv_type, channels[2], channels[3], Ks, gso, bias)
        self.tmp_conv3 = TemporalConvLayer(Kt, channels[3], channels[4], n_vertex, act_func)

        self.tc2_ln = nn.LayerNorm([n_vertex, channels[4]])
        self.relu = nn.ReLU()
