...
def get_parameters():
    ...
    parser.add_argument('--n_his', type=int, default=18)    #default=12
    parser.add_argument('--n_pred', type=int, default=3, help='the number of time interval for predcition, default as 3')
    parser.add_argument('--time_intvl', type=int, default=5)
    parser.add_argument('--Kt', type=int, default=3)
    parser.add_argument('--stblock_num', type=int, default=1)   #default=2, idea 1=1
    ...
    # idea 1: TGT-->TGTGT，stblocksnum = 1
    Ko = args.n_his - (args.Kt) * 2 * args.stblock_num

    blocks = []
    blocks.append([1])
    for l in range(args.stblock_num):
        blocks.append([64, 16, 16, 16, 64])
    if Ko == 0:
        blocks.append([128])
    elif Ko > 0:
...
def data_preparate(args, device):    
    ...
    dataset_path = 'data' #'Documents/Gatech_OMSCS/Courses/CS7643_DeepLearning/CS7643DL_2023Fall/Project/data'
...
def train_and_evaluate(loss, args, optimizer, scheduler, es, model, train_iter, val_iter):
    train_losses = []
    val_losses = []
    ...
    for epoch in range(args.epochs):
        ...
        gpu_mem_alloc = torch.cuda.max_memory_allocated() / 1000000 if torch.cuda.is_available() else 0
        print('Epoch: {:03d} | Lr: {:.20f} |Train loss: {:.6f} | Val loss: {:.6f} | GPU occupy: {:.6f} MiB'.\
            format(epoch+1, optimizer.param_groups[0]['lr'], train_losses[-1], val_loss, gpu_mem_alloc))
...
def plot_losses(train_losses, val_losses):
    ...
    plt.savefig('STGCN Learning Curves-TGTGT-100-his 12.png')
    plt.show()
...
if __name__ == "__main__":
    ...
    train_losses, val_losses = train_and_evaluate(loss, args, optimizer, scheduler, es, model, train_iter, val_iter)
