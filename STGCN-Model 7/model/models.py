...
class STGCNChebGraphConv(nn.Module):
    ...
    def __init__(self, args, blocks, n_vertex):
        ...
        # idea 3: multi attention
        self.attention = nn.MultiheadAttention(embed_dim=207, num_heads=9)   # 1 3 9 23 69 207
        # idea 3: multi attention
    ...
    def forward(self, x):
        ...
        # idea 3: multi attention
        # Reshape x to (sequence_length, batch_size * channels, num_nodes)
        x_for_attention = x.permute(2, 0, 1, 3).contiguous().view(x.size(2), -1, x.size(3))
        # Apply attention
        x_attention, _ = self.attention(x_for_attention, x_for_attention, x_for_attention)
        # Reshape x_attention back to (batch_size, channels, sequence_length, num_nodes)
        x_attention = x_attention.view(x.size(0), x.size(1), x.size(2), x.size(3))
        x = self.st_blocks(x_attention)
        # idea 3: multi attention
